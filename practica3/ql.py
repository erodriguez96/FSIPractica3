...
for i in xrange(100):
    ...
    while state != final_state:
        ...
        if max(Q[state]) > 0:
            ...
            act = getRndAction(state)

        y = getStateCoord(state)[0] + actions_vectors[act][0]
        x = getStateCoord(state)[1] + actions_vectors[act][1]
        new_state = getState(y, x)
        qlearning(state, actions_list[act], new_state)
        state = new_state
print "El promedio es: ", mean/100,"\n"
